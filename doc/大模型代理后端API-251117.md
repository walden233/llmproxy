...
- **REST 版本**：本文档以 `/v1` 命名空间为准；`/v1/v2` 前缀用于 OpenAI 兼容接口。新增的“最近会话”接口挂在 `/v1/conversations/**`。
...
| `modelIdentifier`    | String       | 真实调用 ID，同时用于 `/v2/chat` 的 `model` 字段                                                              |
| `capabilities`       | List<String> | 模型能力，枚举见 `ModelCapabilityEnum`（`text-to-text`、`text-to-image`,`image-to-text`,`image-to-image` 等） |
...
| POST | `/v1/users/assign-role` | `ROLE_ROOT_ADMIN` | 为指定用户分配角色 |
...
### 6.2 用户管理（UserController `/v1/users`）
| 方法 | 路径 | 权限/鉴权 | 说明 |
| ---- | ---- | ---------- | ---- |
| GET | `/v1/users` | `ROLE_ROOT_ADMIN` | 分页查询所有用户 |

#### GET /v1/users
- 逻辑：`IUserService.findAllUsers` 查询所有用户，支持按角色过滤，分页和排序。
- 参数：
| 参数 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- |
| `pageNum` | Integer | 否，默认 1 | 页码 |
| `pageSize` | Integer | 否，默认 10 | 页大小 |
| `role` | String | 否 | 根据用户角色过滤，如 `ROLE_USER`、`ROLE_MODEL_ADMIN` |
| `sortBy` | String | 否，默认 `id` | 排序字段，支持 `id`, `username`, `email`, `role`, `balance` |
| `sortOrder` | String | 否，默认 `desc` | 排序顺序，`asc` 或 `desc` |

### 6.3 Access Key（AccessKeyController `/v1/access-keys`）
| 方法 | 路径 | 权限/鉴权 | 说明 |
| ---- | ---- | ---------- | ---- |
...
### 6.4 供应商与 Provider Key（ProviderController `/v1/providers`）
| 方法 | 路径 | 权限/鉴权 | 说明 |
| ---- | ---- | ---------- | ---- |
...
### 6.5 模型管理（ModelController `/v1/models`）
| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
...
### 6.6 订单与余额（OrderController `/v1/orders`）
| 方法 | 路径 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
...
### 6.7 推理与多模态代理（ProxyController `/v1`）
| 方法 | 路径 | 权限/鉴权 | 说明 |
| ---- | ---- | ---------- | ---- |
| POST | `/v1/chat` | `permitAll` + `ACCESS-KEY` | 自定义聊天接口，返回 `Result<ChatResponse_dto>` |
| POST | `/v2/chat` | `permitAll` + `ACCESS-KEY` | OpenAI `chat.completions` 兼容接口 |
| POST | `/v2/chat/stream` | `permitAll` + `ACCESS-KEY` | SSE 流式聊天 |
...
| GET/POST | `/v1/conversations/**` | `permitAll` + `ACCESS-KEY` | 最近会话列表、消息查询、改名/置顶/删除、创建会话 |
...
| `persistHistory` | Boolean | 否 | 默认 `false`，为 `true` 时后端会自动落地并拼接最近历史 |
| `conversationId` | String | 否 | `persistHistory=true` 时可指定已有会话，不传则自动创建 |

- 逻辑：`langchainProxyService.chat(request, userId, accessKeyId, false)` → 自动扣费/统计 → 若 `persistHistory=true` 会创建/续写会话并拼接最近 10 条持久化历史，返回 `ChatResponse_dto`（回答文本、实际模型、token 使用量、`conversationId`、`messageId`）。
...
| `conversation_id` | String | 否 | 指定已有会话，需与 `persist_history` 配合 |
| `persist_history` | Boolean | 否 | 默认 `false`；为 `true` 时后端落地/拼接历史 |
| `extra_params` | Map | 否 | 厂商扩展参数，如 DashScope 特性 |

- 逻辑：参数转为 LangChain4j `OpenAiChatModel` 请求 → 返回与 OpenAI 官方一致的 `OpenAiChatResponse`，新增 `conversation_id` 字段。流式接口通过 `SseEmitter` 推送 chunk，终止 chunk 会附带 `conversation_id`；异常时返回 `Result` JSON。
...
### 6.9 最近会话与消息查询（ConversationController `/v1/conversations`）
所有接口需 `ACCESS-KEY`，均会校验会话归属。

| 方法 | 路径 | 说明 |
| ---- | ---- | ---- |
| POST | `/v1/conversations` | 创建会话，Body 可选 `{"title": "自定义标题"}`，返回 `conversationId`、`title`、`lastActiveAt` 等 |
| GET | `/v1/conversations/recent?limit=20` | 获取当前用户最近会话列表（按 `pinned` + `lastActiveAt` 倒序），`limit` 1-100 |
| GET | `/v1/conversations/{conversationId}/messages?limit=50&before=2024-01-01T00:00:00` | 拉取会话消息，按时间倒序分页，但响应按时间正序返回便于渲染 |
| PATCH | `/v1/conversations/{conversationId}` | 更新标题/置顶，Body `{"title": "...", "pinned": true}` |
| DELETE | `/v1/conversations/{conversationId}` | 软删除会话 |

消息结构（`ConversationMessageDto`）：`messageId/conversationId/role/content/imageUrls/modelIdentifier/promptTokens/completionTokens/cost/createdAt`。  
列表结构（`ConversationSummaryDto`）：`conversationId/title/pinned/lastModelIdentifier/lastMessageSummary/messageCount/lastActiveAt/createdAt/updatedAt`。

### 6.10 统计与日志（StatisticsController `/v1/statistics`）
| 方法 | 路径 | 权限/鉴权 | 说明 |
| ---- | ---- | ---------- | ---- |
| POST | `/v1/statistics/models` | `ROLE_MODEL_ADMIN` 或 `ROLE_ROOT_ADMIN` | 查询模型日聚合统计（列表，按日期排序），支持不指定模型直接查全部 |
| GET | `/v1/statistics/logs/user/{userId}` | 同上 | Mongo 中某用户的全部 `UsageLogDocument` |
| GET | `/v1/statistics/logs/me` | JWT | `@PreAuthorize("isAuthenticated()")`，普通用户查看自身日志 |
| GET | `/v1/statistics/logs/model/{modelId}` | `ROLE_MODEL_ADMIN` 或 `ROLE_ROOT_ADMIN` | 查看指定模型日志 |
| POST | `/v1/statistics/logs/query` | `ROLE_MODEL_ADMIN` 或 `ROLE_ROOT_ADMIN` | 组合条件查询日志 |

- 模型统计接口返回“模型+日期”聚合的调用次数/成功失败数；日志接口返回单次调用明细。

**`POST /v1/statistics/models` 请求参数**

| 位置 | 字段 | 类型 | 必填 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| body | `StatisticsQueryDto` | Object | 否 | 统计筛选条件 |

**`StatisticsQueryDto` 字段** —— 适用于 `/v1/statistics/models`

| 字段 | 类型 | 必填 | 说明 |
| ----- | ---- | ---- | ---- |
| `modelId` | Integer | 否 | 目标模型主键，不填则不按 ID 过滤 |
| `modelIdentifier` | String | 否 | 目标模型标识，不填则不按标识过滤 |
| `date` | LocalDate | 否 | 单日查询（若提供 `startDate`/`endDate` 则忽略） |
| `startDate` | LocalDate | 否 | 起始日期，缺省时与 `endDate` 对齐 |
| `endDate` | LocalDate | 否 | 结束日期，缺省时与 `startDate` 对齐 |

- 日期规则：优先使用 `startDate`/`endDate` 区间；若都为空且 `date` 提供则查询该日；若都为空则默认当天。会自动按 `statDate` 升序返回。
- 过滤规则：`modelId` 与 `modelIdentifier` 都为空时返回指定日期（或区间）的所有模型统计。
- 响应：`/v1/statistics/models` 返回 `List<ModelStatisticsDto>`，按日期升序（同日按模型 ID）排序，便于直接绘制折线/饼图。
